...
6. ✅ Add `cloudflared` (pinned `2026.9.1`, token from `.env`, on `mcnet`) and
   ...
   only by devices running the WARP client and pulls in split-tunnel and CGNAT
   configuration, while a public hostname creates the DNS record itself and is
   reachable by anyone. If the dashboard starts talking about
   `100.64.0.0/10` or Local Domain Fallback, it is the wrong one.
